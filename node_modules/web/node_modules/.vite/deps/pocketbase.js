  ...
  initConnect() {
    this.disconnect(true), clearTimeout(this.connectTimeoutId), this.connectTimeoutId = setTimeout((() => {
      ...
      this.clientId = t2?.lastEventId, this.lastSentSubscriptions = [], this.submitSubscriptions().then((async () => {
        let t3 = 3;
        for (; this.hasUnsentSubscriptions() && t3 > 0; ) t3--, await this.submitSubscriptions();
        ...
        for (let t4 in s2) for (let i2 of s2[t4]) i2(e2);
...
RegExp.escape || (RegExp.escape = function(e2) {
  return e2.replace(/[/\-\\^$*+?.()|[\]{}]/g, "\\$&");
});
var Client = class {
  get baseUrl() {
  ...
  filter(e2, t2) {
    if (!t2) return e2;
    const s2 = {};
    for (const e3 in t2) s2["{:" + e3 + "}"] = t2[e3];
    const i2 = Object.keys(s2).map(((e3) => RegExp.escape(e3))).join("|");
    if (!i2) return e2;
    return e2.replace(new RegExp(i2, "g"), ((e3) => (function stringify(e4) {
      switch (typeof e4) {
        case "boolean":
        case "number":
          return "" + e4;
        ...
        default:
          ...
          if (e4 instanceof Date) return JSON.stringify(e4.toISOString().replace("T", " "));
          const t3 = JSON.stringify(e4);
